...
# img = cv2.imread(f"./assets/data-resampled/clock_0740.png")
...
# Crear máscara para detectar lo oscuro (manecillas)
mask = (
  (R < 125) & 
  (G < 125) & 
  (B < 125)
...
# Buscar contornos en la máscara reducida
...
for cnt in contours:
    ...
    # Encontrar el punto más lejano al centro (punta de la manecilla)
    ...
    dx = tip[0] - x_center
    dy = y_center - tip[1]  # invertir Y para coordenadas de reloj
...
# Separar minutero y hora según longitud (opcional si tienes distancias)
angles = sorted(angles, key=lambda x: np.hypot(x[1][0]-x_center, x[1][1]-y_center), reverse=True)
...
# Ajuste suave de horas según minutos
hours += (minutes/60)
...
# plt.subplot(1,3,3); plt.title("Rectas ajustadas"); plt.imshow(img_lines); plt.axis("off")
plt.show()
